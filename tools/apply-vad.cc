...
int main(int argc, char *argv[]) {
    ...
    option.Register("silence-thresh", &silence_thresh, "threshold of silence(0 ~ 1)");
    ...
    int min_length = 50; 
    option.Register("min-length", &min_length, "Minimum length of the voice segment");
    int max_length = 1000;
    option.Register("max-length", &max_length, "Maximum length of the voice segment");
    ...
    vad_config.net_file = net_file; //"../model/kws.net"
    ...
    while (cur < results.size()) {
        ...
        int start = cur;
        while (cur < results.size() && cur - start < max_length && results[cur]) cur++;
        int end = cur;
